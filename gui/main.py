...
class Field:
    ...
    def __init__(
        ...
        self.box.fill(color)
        self.rect: pygame.Rect = self.box.get_rect()
...
class TextField(Field):
    ...
    def updateText(self, text):
        self.font.render(text, True, Colors.WHITE)
        self.rect = self.box.get_rect()
...
class Button(Field):
    ...
    def __init__(
        self,
        button_released: Field,
        button_pressed: Field,
        textField: TextField,
        action: callable = ButtonActions.doNothing,
    ):
        self.button_released = button_released
        self.field2 = button_pressed
        self.z = button_released.z
        self.state = 1
        self.rect = self.field.rect
        self.rect = button_released.rect
        self.center = button_released.rect.center
        ...
        if Game.State == Game.Player and len(Game.asyncTasks) == 0:
            for k, v in bRepr.tiles.items():
                if v.rect.collidepoint(mousePos):
                    ...
                    if (
                        s == None
                        and not bRepr.isTileEmpty(k)
                        and bRepr.getColor(v.pos) == bRepr.board.ctm
                    ):
                        v.onClick()
    ...
    def createBRepr(self):
        ...
        for pos in range(64):
            if (Utils.rank(pos) + Utils.col(pos)) % 2 == 1:
                self.tiles[pos] = TileSprite(pos, Colors.WHITE)
            else:
                self.tiles[pos] = TileSprite(pos, Colors.GREY)
...
class UI:
    ...
    def wCheckMate(self):
        ...
        self.fields = []
        txt1 = TextField(
            "CheckMate :)", (x + 2, y - 62), self.font1, color=Colors.GREEN
        )
        self.addField(txt1)
        txt2 = TextField(
            "CheckMate :)", (x, y - 60), self.font1, color=Colors.LIGHTBLUE
        )
    ...
    def createMenu(self):
        ...
        txt1 = TextField("CHESS", (x + 2, y - 62), self.font1, color=Colors.GREEN)
        self.addField(txt1)
        txt2 = TextField("CHESS", (x, y - 60), self.font1, color=Colors.LIGHTBLUE)
        self.addField(txt2)
